    ...
    <el-card>
      <el-table :data="recordList" stripe style="width: 100%">
        ...
        <el-table-column label="操作" width="180"> 
          <template slot-scope="scope">
            <el-button @click="agree(scope.row)" type="success" icon="el-icon-check" size="mini" circle></el-button>
            <el-button @click="disagree(scope.row)" type="danger" icon="el-icon-close"  size="mini" circle></el-button>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    agree(record){
      recordApi.agreeRefund(record.recordId).then(response => {
      this.$message({
              type: 'success',
              message: "同意退费"
            });
    ...
    disagree(record){
      recordApi.disagreeRefund(record.recordId).then(response => {
      this.$message({
              type: 'success',
              message: "不同意退费"
            });
    ...
    getRefundList() {
      recordApi.getRefundList(this.searchModel).then((response) => {
