    ...
    <el-card>
      <el-table :data="recordList" stripe style="width: 100%">
        ...
        <el-table-column prop="time" label="消费日期">
         ...
         <el-table-column prop="refund" label="退费状态">
          <template slot-scope="scope">
            <el-tag v-if="scope.row.refund == 1">正常</el-tag>
            <el-tag v-else-if="scope.row.refund == 2" type="warning">审核中</el-tag>
        ...
        <el-table-column label="操作" width="180"> 
          <template slot-scope="scope">
            ...
            <el-button @click="refund(scope.row)" type="danger" icon="el-icon-coin" size="mini" circle></el-button>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    refund(record){
      this.$confirm(`您确认申请退费消费原因为 ${record.reason} 金额为 ${record.cost} 的这条消费记录 ?`, '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        }).then(() => {
          recordApi.refundRecordById(record.recordId).then(response => {
            this.$message({
              type: 'success',
              message: response.message
            });
    ...
    getRecordList() {
      recordApi.getSingleRecordList(this.searchModel,this.id).then((response) => {
        this.recordList = response.data.rows;
        this.total = response.data.total;
      });
    },
    exportExcel(){
      recordApi.exportSingleExcel(this.id);
      this.$message({
              type: 'success',
              message: "导出成功"
            });
